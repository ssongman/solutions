...
Loki는 대규모 로그 수집 및 분석을 위한 분산형 로그 관리 시스템으로, 여러 구성 요소(Pod)로 이루어져 있다. 각 구성 요소는 특정 역할을 수행하며, 전체 시스템이 원활하게 동작하도록 협력한다. 아래는 Loki를 구성하는 주요 Pod들의 역할에 대한 설명이다.

### 1. **Ingester**
   - **역할**: Ingester는 Loki에서 로그를 실제로 저장하는 역할을 한다. 로그 데이터를 수신하고, 이를 메모리와 저장소에 보관한다. 새로운 로그가 들어오면 이 로그를 인덱싱하고, 설정된 보존 기간이 지나면 이를 삭제한다.
   - **특징**: Ingester는 로그 데이터의 일관성을 유지하고, 시스템 장애 시 데이터를 복구할 수 있도록 데이터를 분산 저장한다.

### 2. **Querier**
   - **역할**: Querier는 Loki의 쿼리 엔진 역할을 한다. 사용자나 애플리케이션에서 오는 로그 쿼리를 처리하며, 필요한 로그 데이터를 검색하고 반환한다.
   - **특징**: Querier는 대량의 데이터를 빠르게 검색할 수 있도록 최적화되어 있으며, 쿼리 결과를 효율적으로 제공하기 위해 인덱싱된 데이터를 활용한다.

### 3. **Query Frontend**
   - **역할**: Query Frontend는 사용자의 쿼리를 수신하고, 이를 Querier에게 전달하는 역할을 한다. 쿼리 요청을 처리하는 데 있어 캐싱, 페이징, 결과 집계를 지원하며, 쿼리 성능을 향상시킨다.
   - **특징**: Query Frontend는 여러 쿼리 요청을 병렬로 처리하여 성능을 최적화하고, 복잡한 쿼리를 효율적으로 실행할 수 있도록 한다.

### 4. **Query Scheduler**
   - **역할**: Query Scheduler는 쿼리 작업을 스케줄링하고 분산하는 역할을 한다. 쿼리 요청이 여러 개의 Querier에 효율적으로 분배되도록 관리하며, 시스템 전체의 부하를 조정한다.
   - **특징**: 이 구성 요소는 대규모 쿼리 요청 처리 시 성능을 극대화하기 위해 필수적이다.

### 5. **Distributor**
   - **역할**: Distributor는 로그 데이터를 수집하는 역할을 하며, 수집된 로그를 인제스터(Ingester)로 분배한다. 로그 데이터를 수신한 후, 이를 여러 인제스터에 분산하여 저장한다.
   - **특징**: 로그의 수집 및 분산 과정을 담당하며, 시스템의 가용성과 확장성을 보장한다.

### 6. **Compactor**
   - **역할**: Compactor는 로그 데이터를 압축하고, 오래된 데이터를 정리하는 역할을 한다. 이 구성 요소는 시간이 지나면서 축적된 로그 데이터를 효율적으로 관리하기 위해 필요한다.
   - **특징**: Compactor는 저장소의 효율성을 높이고, 데이터를 압축하여 디스크 사용량을 줄이며, 오래된 데이터의 보존 정책을 적용한다.

### 7. **Index Gateway**
   - **역할**: Index Gateway는 Loki의 인덱스 데이터를 관리하는 구성 요소이다. 로그 데이터를 더 쉽게 검색할 수 있도록 인덱스를 생성하고, 이를 관리한다.
   - **특징**: 효율적인 인덱스 관리를 통해 쿼리 성능을 향상시키고, 데이터 조회 속도를 개선한다.

이들 구성 요소는 함께 작동하여 Loki가 대규모 로그 데이터를 효과적으로 수집, 저장, 분석할 수 있도록 지원한다. 각 구성 요소는 특정 역할에 최적화되어 있으며, 전체 시스템의 성능과 확장성을 보장하는 데 중요한 역할을 한다.
...
#### 1.1 Helm 설치 (필요한 경우)
Helm이 설치되어 있지 않다면, 아래 명령어로 설치한다.
...
Grafana Labs에서 제공하는 Helm 차트를 사용해 LGTM 스택을 설치할 수 있다. Grafana의 Helm 리포지토리를 추가하고 업데이트한다.
...
Grafana는 시각화 도구로, 다양한 데이터 소스를 연결할 수 있다. Grafana를 설치하려면 아래 명령어를 사용한다.
...
### (3) Grafana 대시보드 접속
브라우저에서 `http://<External-IP>:<Port>`로 Grafana에 접속하고, `admin` 사용자와 위에서 확인한 비밀번호로 로그인한다.
...
이 에러 메시지는 Grafana가 Loki와 통신할 때 “no org id”라는 오류 메시지를 반환받아 발생한 것이다. 이 문제는 보통 Loki의 멀티테넌시 설정 또는 Grafana와 Loki 간의 인증 문제에서 발생할 수 있다.
